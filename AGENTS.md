...
**给 AI 助手的执行提示**：`server.py` 是**前台常驻进程**，直接在会话里执行会一直阻塞。要么让用户自己开一个终端窗口跑（推荐），要么放到后台执行；跑完记得用 `curl -s -o /dev/null -w "%{http_code}" http://localhost:8131/index.html` 确认返回 200，而不是干等。端口被占用时先 `lsof -ti :8131` 查。
