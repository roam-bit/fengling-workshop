...
手机可直接打开（建议横屏）；编辑器请用电脑。**在线版能看到几乎全部内容，但改动存不下来**——详见下面的对比表。
...
不装本地也能看到**绝大部分**，只有「写操作」需要本地 server（GitHub Pages 是纯静态托管，没有后端）：

| 功能 | 在线（GitHub Pages） | 本地跑 |
|---|---|---|
| 平台首页、项目卡 | ✅ 完整 | ✅ |
| **玩游戏**：世界地图 / 4 篇章节 / NPC 对话 / 线索 / 道具 / 机制触发 | ✅ **100% 完整** | ✅ |
| **编辑器界面**：打开、拖物件、点 Inspector、看机制卡 / 语义层 / 模板 | ✅ 完整可见可操作 | ✅ |
| 💾 **保存改动** | ❌ 提示「server 未运行」，刷新即失 | ✅ 写回 `chapters.json` |
| ⌘ **AI 自然语言命令** | ❌ 需后端调用模型 | ✅ |
| ➕ **新建项目 / 套用模板** | ❌ 需后端写文件 | ✅ |
| 🧪 E2E 自动化测试 | ❌ | ✅ |

一句话：**在线随便看、随便玩；想真正动手改，clone 下来跑。**
